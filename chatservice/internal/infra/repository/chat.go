...
import (
	...
	"github.com/eduardotecnologo/FCLX/chatservice/internal/domain/entity"
	"github.com/eduardotecnologo/FCLX/chatservice/internal/infra/db"
)
...
func (r *ChatRepositoryMySQL) CreateChat(ctx context.Context, chat *entity.Chat) error {
	err := r.Queries.CreateChat(
		...
		db.CreateChatParams{
			ID:               chat.ID,
			UserID:           chat.UserID,
			InitialMessageID: chat.InitialSistemMessage.Content,
			Status:           chat.Status,
	...
	err = r.Queries.AddMessage(
		ctx,
		db.AddMessageParams{
			ID:        chat.InitialSistemMessage.ID,
			ChatID:    chat.ID,
			Content:   chat.InitialSistemMessage.Content,
			Role:      chat.InitialSistemMessage.Role,
			Tokens:    int32(chat.InitialSistemMessage.Tokens),
			CreatedAt: chat.InitialSistemMessage.CreatedAt,
		},
	)
	if err != nil {
		return err
